...
test("v2.5 reading migration is additive and preserves legacy columns", () => {
  ...
  assert.deepEqual(journal.entries.slice(0, 11).map((entry) => entry.tag), [
    ...
    "0004_typesafe_pilot", "0005_user_isolation", "0006_database_memory_search", "0007_narrative_verification", "0008_agreement_events", "0009_narrative_evidence_search", "0010_narrative_diagnostics",
  ]);
});

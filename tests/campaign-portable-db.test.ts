...
test("portable HTTP export is owner-only and import remaps structured data without copying jobs or credentials", async () => {
  ...
  try {
    ...
    await fixture.pg.query("INSERT INTO game_sessions(id,owner_id,visibility,title,character,world_state,turn_count) VALUES ($1,$2,'public','Harbor',$3,$4,1)", [source, owner, JSON.stringify(character), JSON.stringify(world)]);
    ...
    const copiedTurn = (await fixture.pg.query<{ id: string; request_id: string | null }>("SELECT id,request_id FROM game_turns WHERE session_id=$1", [result.session.id])).rows[0];

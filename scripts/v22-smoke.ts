...
async function run() {
  ...
  const status = await call<{ version: string; database: string }>("/api/system/status"); assert.equal(status.status, 200); assert.equal(status.data.version, "2.7"); assert.equal(status.data.database, "connected");

...
const smokeKeyring = installSyntheticSecretKeyring("legacy-smoke-v1");
...
async function run() {
  ...
  const legacyGemini = sealSecret("fake-legacy-secret-gemini", secretContext("global", "gemini"), smokeKeyring);
  const legacyTypeSafe = sealSecret("fake-legacy-secret-typesafe", secretContext("global", "typesafe-pilot"), smokeKeyring);
  const legacyNarrative = sealSecret("fake-legacy-secret-narrative", secretContext("global", "narrative:openrouter"), smokeKeyring);
  await pool.query("insert into ai_settings(id,keys,typesafe_key,narrative_guard_provider,narrative_guard_key,use_live_ai) values('global',$1,$2,'openrouter',$3,false)", [JSON.stringify([legacyGemini]), legacyTypeSafe, legacyNarrative]);
  ...
  const importedAI = (await pool.query("select keys,typesafe_key,narrative_guard_key from ai_settings where id=$1", [profiles[0]])).rows[0];
  assert.equal(openSecret(importedAI.keys[0], secretContext(profiles[0], "gemini"), { keyring: smokeKeyring }), "fake-legacy-secret-gemini");
  assert.equal(openSecret(importedAI.typesafe_key, secretContext(profiles[0], "typesafe-pilot"), { keyring: smokeKeyring }), "fake-legacy-secret-typesafe");
  assert.equal(openSecret(importedAI.narrative_guard_key, secretContext(profiles[0], "narrative:openrouter"), { keyring: smokeKeyring }), "fake-legacy-secret-narrative");
  ...
  const existingNarrative = sealSecret("fake-target-existing-narrative", secretContext(profiles[1], "narrative:typesafe"), smokeKeyring);
  await pool.query("update ai_settings set narrative_guard_provider='typesafe', narrative_guard_key=$2 where id=$1", [profiles[1], existingNarrative]);
  await invoke([`--profile=${profiles[1]}`, "--include-settings", "--apply"], /already has credentials/);
  assert.equal((await pool.query("select narrative_guard_key from ai_settings where id=$1", [profiles[1]])).rows[0].narrative_guard_key, existingNarrative);
  ...
  assert.equal((await pool.query("select narrative_guard_key from ai_settings where id=$1", [profiles[1]])).rows[0].narrative_guard_key, existingNarrative, "Campaign-only import leaves target settings intact");

...
Миграции — отдельный шаг развёртывания, они не запускаются при старте HTTP-сервера. Drizzle ledger учитывает семь версий: `0000` — базовая схема, `0001` — надёжность пространства, `0002` — ветки и задания, `0003` — настройки чтения, `0004` — настройки и отчёты пилота TypeSafe, `0005` — личные профили и владение кампаниями, `0006` — точный поиск памяти через pgvector и pgcrypto. Повторный запуск пропускает применённые версии. Для обновления существующей базы сначала сделайте резервную копию; `drizzle-kit push` не заменяет историю миграций.

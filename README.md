...
Состояние документации: **23 сентября 2026**, включая личные профили, потоковые ходы, проверку повествования Jev, журнал договорённостей и фоновую диагностику. Актуальные приоритеты — в [roadmap](docs/superpowers/plans/roadmap.md); документы отдельных версий сохраняются как история.
...
По умолчанию миграции — отдельный шаг развёртывания. Явный `CHRONICLE_AUTO_MIGRATE=1` включает их перед подготовкой web/worker; ошибка завершает процесс. Drizzle ledger учитывает четырнадцать версий: `0000` — базовая схема, `0001` — надёжность пространства, `0002` — ветки и задания, `0003` — настройки чтения, `0004` — пилот TypeSafe, `0005` — личные профили и кампании, `0006` — поиск памяти через pgvector/pgcrypto, `0007` — настройки проверки повествования, `0008` — журнал договорённостей, `0009` — полнотекстовый индекс исходных ходов, `0010` — диагностика попыток, `0011` — аккаунты и административные границы, `0012` — атомарные квоты, `0013` — идемпотентный импорт кампаний. Повторный запуск пропускает применённые версии. `0009` заполняет вычисляемое поле существующих записей под блокировкой таблицы; длительность зависит от объёма истории. Для обновления существующей базы сначала сделайте резервную копию; `drizzle-kit push` не заменяет историю миграций.
...
Следующие приоритеты: развитие взаимодействий и состояния мира, эксплуатационные проверки и восстановление; справедливые очереди и provider concurrency под измеренной нагрузкой; приближённый векторный индекс при подтверждённой необходимости; recall@K/genre drift/entailment evaluation; затем authored scene graphs и мультимодальная память. NDJSON streaming уже реализован; требуется измерение задержки и качества на реальных моделях. Произвольное branch-from-turn потребует event log и replay: текущие контрольные точки этого не имитируют.

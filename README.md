...
Состояние документации: **20 сентября 2026**, включая личные профили, потоковые ходы, проверку повествования Jev, журнал договорённостей и фоновую диагностику. Актуальные приоритеты — в [roadmap](docs/superpowers/plans/roadmap.md); документы отдельных версий сохраняются как история.
...
В настройках есть отдельный раздел **«Для разработки»**: Jev `jev-1.13.0` наблюдает за качеством извлечённых фактов, но не меняет их сохранение. Пилот выключен по умолчанию, использует личный ключ из UI и независим от проверки повествования. Подробности и оценочные наборы: [эксплуатация и проверки](docs/story-experience-operations.md).
...
Диагностика успешных, остановленных и непроверенных попыток записывается через Next.js `after`, после отправки ответа: ход не ждёт диагностическую БД. Доступ — только владельцу кампании, окно чтения — 30 дней. Аварийное завершение процесса может потерять последнюю диагностическую запись; результат игры хранится отдельно. [Включение, разбор ошибок и оставшиеся ограничения](docs/narrative-verification-release.md).
...
Миграции — отдельный шаг развёртывания, они не запускаются при старте HTTP-сервера. Drizzle ledger учитывает одиннадцать версий: `0000` — базовая схема, `0001` — надёжность пространства, `0002` — ветки и задания, `0003` — настройки чтения, `0004` — пилот TypeSafe, `0005` — личные профили и кампании, `0006` — поиск памяти через pgvector/pgcrypto, `0007` — настройки проверки повествования, `0008` — журнал договорённостей, `0009` — полнотекстовый индекс исходных ходов, `0010` — диагностика попыток. Повторный запуск пропускает применённые версии. `0009` заполняет вычисляемое поле существующих записей под блокировкой таблицы; длительность зависит от объёма истории. Для обновления существующей базы сначала сделайте резервную копию; `drizzle-kit push` не заменяет историю миграций.

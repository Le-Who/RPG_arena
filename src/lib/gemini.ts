...
export function buildNarrationSystemPrompt(opts: {
  ...
  const header = `Ты — Гейм-мастер текстовой RPG в духе D&D (русский язык, второе лицо, кинематографично: 110–200 слов).
...
Верни СТРОГО валидный JSON без текста вне него:
{
  ...
  "character_updates": { "facts": ["Изменение HP/статуса/навыков", "Новый предмет/потеря"] },
  "world_updates": { "facts": ["Изменение в мире, фракции, локации"] },
  "flags": { "ключ": "значение" },
...
Канон: не изменяй имена, не противоречь фактам предыдущей памяти. Русский язык.`;
}

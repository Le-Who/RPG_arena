...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = await db.select().from(gameTurns).where(and(eq(gameTurns.sessionId, id), lt(gameTurns.turnNumber, before))).orderBy(desc(gameTurns.turnNumber), desc(gameTurns.createdAt)).limit(60);

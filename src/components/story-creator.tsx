"use client";
import { STORY_SHAPE_LABELS, type StoryShapeKind } from "@/lib/world-life";
...
export function StoryCreator({ initialScenario, initialMode, onClose, onCreated, live, onSettings, suspended = false }: StoryCreatorProps) {
  ...
  const [storyKind, setStoryKind] = useState<StoryShapeKind>("open-life");
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const body = mode === "preset" ? { mode, scenarioId, characterIndex, storyShape: { kind: "arc" } } : { mode, storyShape: { kind: storyKind, ...(storyKind === "arc" ? { goal: form.mainQuest } : {}) }, rulesProfile: form.rulesProfile, customScenario: { title: form.title, worldName: form.worldName || form.title, pitch: form.pitch, era: form.era, tone: form.tone, mainQuest: form.mainQuest, startLocation: form.startLocation }, customCharacter: { name: form.name, archetype: form.archetype, backstory: form.backstory, skills: form.skills.split(",").map((s) => s.trim()).filter(Boolean), startItems: form.startItems.split(",").map((s) => s.trim()).filter(Boolean) } };
      const result = await api<{ session: Session }>("/api/sessions", jsonBody({ ...body, visibility: published ? "public" : "private" }));
